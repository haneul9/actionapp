...
common.MEmployeeProfile1 = {
	...
	*/	
	...
	makeBasicProfile : function(oController, Pernr) {
		...
		var oEmpBasic = new control.L2PEmpBasic(oController.PAGEID + "_EmpBasicControl", {
	...
	makeDetailInfo : function(oController, Pernr) {
		...
		var oMasterTabLayout = new control.L2PTab({
			...
			select : common.MEmployeeProfile1.onSelectMasterTabMenu,
			bgColor : "#53abe6"
		});		
		...
		if(oController._vMasterTabs[0].childControl == "5" || oController._vMasterTabs[0].childControl == "7") {
			common.MEmployeeProfile1.makeListData(oController, Pernr, oController._vMasterTabs[0].id, oController._vMasterTabs[0].childControl, oController._vMasterTabs[0].merge);
		} else if(oController._vMasterTabs[0].childControl == "1") {
			common.MEmployeeProfile1.makeSubTabMenu(oController, Pernr, oController._vMasterTabs[0].id, "1");
		} else if(oController._vMasterTabs[0].childControl == "2") {
			common.MEmployeeProfile1.makeSubTabMenu(oController, oController._vSelectedPernr, oController._vMasterTabs[0].id, "2");
			common.MEmployeeProfile1.makeDataSet(oController, oController._vSelectedPernr, oController._vMasterTabs[0].id);
		}
	},
	...
	makeListData : function(oController, Pernr, Menuid, MenuType, MenuMerge) {
		...
		common.MEmployeeProfile1.vTableType = MenuType;
		common.MEmployeeProfile1.vTableMenuId = Menuid;
		common.MEmployeeProfile1.vController = oController;
		...
		createIBSheet2(document.getElementById(oController.PAGEID + "_DHtmlxTable"), "EmpolyeeProfileTable1", "100%",  (vTableHeight) + "px", IBSHeet_Locale);
		
		EmpolyeeProfileTable1.Reset();
		
		EmpolyeeProfileTable1.SetTheme("TT", "TopTeam");
		...
		common.MEmployeeProfile1.vTableHeader = [];
		oModel.read("/TableHeaderSet/?$filter=Pernr%20eq%20%27" + Pernr + "%27" +
				...
				function(oData, oResponse) {					
					if(oData && oData.results.length) {
						for(var i=0; i<oData.results.length; i++) {
							common.MEmployeeProfile1.vTableHeader.push(oData.results[i]);
						}
					}
				},
				function(oResponse) {
		...
		if(common.MEmployeeProfile1.vTableHeader.length < 1) {
			return;
		}
		
		var c_w = Math.floor(100 / common.MEmployeeProfile1.vTableHeader.length);
		
		for(var i=0; i<common.MEmployeeProfile1.vTableHeader.length; i++) {
			var oneCol = {};
			oneCol.Header = common.MEmployeeProfile1.vTableHeader[i].Header;
			if(common.MEmployeeProfile1.vTableHeader[i].Fldty == "50") {
				oneCol.Type = "Img";
			} else if(common.MEmployeeProfile1.vTableHeader[i].Fldty == "80") {
				oneCol.Type = "Html";
			} else {
				oneCol.Type = "Text";
			}
		...
		IBS_InitSheet(EmpolyeeProfileTable1, initdata);
		EmpolyeeProfileTable1.FitColWidth();
		EmpolyeeProfileTable1.SetSelectionMode(0);
		
		EmpolyeeProfileTable1.SetCellFont("FontSize", 0, "Value01", EmpolyeeProfileTable1.HeaderRows(), EmpolyeeProfileTable1.LastCol(), 12);
		EmpolyeeProfileTable1.SetCellFont("FontName", 0, "Value01", EmpolyeeProfileTable1.HeaderRows(), EmpolyeeProfileTable1.LastCol(), "Malgun Gothic");
		EmpolyeeProfileTable1.SetCellFont("FontBold", 0, "Value01", EmpolyeeProfileTable1.HeaderRows(), EmpolyeeProfileTable1.LastCol(), 1);
		EmpolyeeProfileTable1.SetHeaderRowHeight(32);
		EmpolyeeProfileTable1.SetDataRowHeight(32);
		
		EmpolyeeProfileTable1.SetHeaderBackColor("rgb(255,255,255)");
		EmpolyeeProfileTable1.SetFocusAfterProcess(0);
		
		
		if(MenuType == "7") {
			EmpolyeeProfileTable1.SetMergeSheet(1);
		} else {
			EmpolyeeProfileTable1.SetMergeSheet(0);
		}
		...
		EmpolyeeProfileTable1.LoadSearchData(vPersonListData);		
	},
	...
	onSelectMasterTabMenu : function(oEvent) {
		...
		if(vMenuControlType == "5") {
			oSubTabMenuLayout.setVisible(false);
			common.MEmployeeProfile1.makeListData(oController, oController._vSelectedPernr, vMenuId);
		} else if(vMenuControlType == "1") {
			common.MEmployeeProfile1.makeSubTabMenu(oController, oController._vSelectedPernr, vMenuId, "1");
		} else if(vMenuControlType == "2") {
			common.MEmployeeProfile1.makeSubTabMenu(oController, oController._vSelectedPernr, vMenuId, "2");
			common.MEmployeeProfile1.makeDataSet(oController, oController._vSelectedPernr, vMenuId);
		}
	...
	makeSubTabMenu : function(oController, Pernr, MenuId, vMenuControlType) {
		...
		var oSubTabLayout = new control.L2PTab({
			...
			bgColor : "#FFFFFF",
			select : common.MEmployeeProfile1.onSelectSubTabMenu
		});
		...
		if(vMenuControlType == "1") common.MEmployeeProfile1.makeListData(oController, Pernr, vFirstMenuId, vFirstType, vFirstMerge);
	},
	...
	onSelectSubTabMenu : function(oEvent) {
		...
		if(vMenuControlType == "5" || vMenuControlType == "7" || vMenuControlType == "8") {
			common.MEmployeeProfile1.makeListData(oController, oController._vSelectedPernr, vMenuId, vMenuControlType, vMenuMerge);
...
function EmpolyeeProfileTable1_OnSearchEnd(result) {
	if(common.MEmployeeProfile1.vTableType != "8") {
		for(var i=0; i<common.MEmployeeProfile1.vTableHeader.length; i++) {
			if(common.MEmployeeProfile1.vTableHeader[i].Width != "" && parseInt(common.MEmployeeProfile1.vTableHeader[i].Width) > 0) {
				EmpolyeeProfileTable1.SetColWidth(i, parseInt(common.MEmployeeProfile1.vTableHeader[i].Width));
			}
		}
...
function EmpolyeeProfileTable1_OnSmartResize(Width, Height) {
	if(common.MEmployeeProfile1.vTableType != "8") {
		for(var i=0; i<common.MEmployeeProfile1.vTableHeader.length; i++) {
			if(common.MEmployeeProfile1.vTableHeader[i].Width != "" && parseInt(common.MEmployeeProfile1.vTableHeader[i].Width) > 0) {
				EmpolyeeProfileTable1.SetColWidth(i, parseInt(common.MEmployeeProfile1.vTableHeader[i].Width));
