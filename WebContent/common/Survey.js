...
common.Survey = {
	...
	initSurvey : function(oController, ty) {
		...
		common.Survey.SurveyInfo.push({id: "SURVEY_2_1_1", label : "회사는 공표된 비전과 가치에 따라 운영된다", value : "", requried : true});
		...
		common.Survey.SurveyInfo.push({id: "SURVEY_2_2_1", label : "회사의 보상수준은 동종 업계와 비교했을 때 높은 수준이다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_2_2", label : "회사는 개인의 공헌도에 따라 공정하게 보상을 한다", value : "", requried : true});
		...
		common.Survey.SurveyInfo.push({id: "SURVEY_2_3_2", label : "팀의 동료들은 직무성과를 높일 수 있는 새로운 정보에 대해 공유한다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_3_3", label : "퇴대부분의 동료들을 나를 잘 지원해 주었다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_4_1", label : "나의 상사는 내 역량을 발휘할 수 있는 기회를 제공해 주었다.", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_4_2", label : "나의 상사는 나의 업무성과에 대하여 적절한 피드백을 해주었다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_4_3", label : "나는 전반적으로 상사를 믿고 따를 수 있었다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_5_1", label : "나에게 주어진 직무의 중요도에 대해 만족감을 느낄 수 있었다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_5_2", label : "나의 직무는 나의 성장과 발전에 도움이 되었다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_5_3", label : "나는 내가 맡은 업무에 열정에 가지고 있다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_6_1", label : "업무 때문에 원하는 만큼 가족과 시간을 보낼 수 없었다", value : "", requried : true});
		common.Survey.SurveyInfo.push({id: "SURVEY_2_6_2", label : "업무 시간으로 인해 집안일을 할 시간이 부족하다.", value : "", requried : true});
		...
		if(ty == "V") {
			...
			var oPath = "/RetirementSurveySet/?$filter=Appno%20eq%20%27" + oController._vAppno + "%27"
	...
	SurveyRadio3 : function(oEvent) {
		...
		var oInputs = [];
		oInputs.push([oController.PAGEID + "_SURVEY_3_1_1", oController.PAGEID + "_SURVEY_3_1_2", oController.PAGEID + "_SURVEY_3_1_3", oController.PAGEID + "_SURVEY_3_1_4"]);
		...
		for(var i=0; i<common.Survey.SurveyInfo.length; i++) {
			...
			if(common.Survey.SurveyInfo[i].id == "SURVEY_3") {
				if(common.Survey.SurveyInfo[i].value == "1") {
					for(var j=0; j<common.Survey.SurveyInfo.length; j++) {
						if(common.Survey.SurveyInfo[j].id == "SURVEY_3_1_1"
					...
					for(var j=0; j<common.Survey.SurveyInfo.length; j++) {
						if(common.Survey.SurveyInfo[j].id == "SURVEY_3_2_1"
					...
					for(var j=0; j<common.Survey.SurveyInfo.length; j++) {
						if(common.Survey.SurveyInfo[j].id == "SURVEY_3_3_1"
							|| common.Survey.SurveyInfo[j].id == "SURVEY_3_3_2") {
							if(common.Survey.SurveyInfo[j].value == "") {
								sap.m.MessageBox.alert(vMsg.replace("&ITEM&", common.Survey.SurveyInfo[j].label));
								return;
							}
