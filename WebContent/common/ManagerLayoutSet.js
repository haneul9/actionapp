...
common.ManagerLayoutSet = {
	...
	*/	
	...
	managerLayoutSet : function(oController) {
		...
		oMpanel.addContent(new sap.m.Toolbar({
			height : "30px",
		...
		oModel.read("/GetManagingStaffSet" + filterString , 
					null, 
					null, 
					false,
						...
						if(oData && oData.results.length > 0) {
							...
							for(var i=0; i<oData.results.length; i++) {
								oMpanel.addContent(
									new sap.m.Toolbar({
										height : "21px",
										design : sap.m.ToolbarDesign.Auto,
										content : [new sap.ui.core.Icon({src : "sap-icon://hr-approval", size : "1.0rem", color : "#666666"}),
							...
							oMpanel.addContent(
									new sap.m.Toolbar({
										height : "21px",
										design : sap.m.ToolbarDesign.Auto,
										content : [new sap.m.Label({text : "현재 신청관리 담당자가 등록되어 있지 않습니다."}).addStyleClass("L2P13Font")]
									}).addStyleClass("L2PToolbarNoBottomLine L2PPaddingLeft2rem")
