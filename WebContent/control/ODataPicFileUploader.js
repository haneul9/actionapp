...
sap.ui.unified.FileUploader.extend("control.ODataPicFileUploader", {
	...
	upload : function() {
        ...
        if(vBrowserInfo != "") {
        	...
        	if(vTemp[0].toLowerCase() == "msie") {
            	if(parseInt(vTemp[1]) < 10) {
            		this.fireUploadComplete({"response": "Error: ���� ���ε� ����� IE 10 �̻���� �����մϴ�."});
            		return;
            	}
            }
        }
		...
		var _handleSuccess = function(data){
			that.fireUploadComplete({"response": "파일 업로드를 완료하였습니다."});
...
//							"slug": encodeURI(file.name).split(".")[0] + "|" + encodeURI(file.name) + "|" + encodeURI(file.type)
				...
				that.fireUploadComplete({"response": "Error: " + "���ε� ������ �������� ���߽��ϴ�."});
				that._bUploading = false;
			}
		} catch(oException) {
			that.fireUploadComplete({"response": "파일 업로드에 실패하였습니다." +" :\n" + oException.message});
			that._bUploading = false;
